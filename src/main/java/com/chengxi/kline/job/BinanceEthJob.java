...
public class BinanceEthJob {
    ...
    @Autowired
    private EthCommonDataMapper ethCommonDataMapper;

    @PostConstruct
    private void doJob(){
        taskExecutor.execute(new EthSpiderJob(ethCommonDataMapper));
    }

    static class EthSpiderJob implements Runnable{

        EthCommonDataMapper mapper;

        EthSpiderJob(EthCommonDataMapper ethCommonDataMapper){
            mapper = ethCommonDataMapper;
        }
        ...
        public void run() {
            ...
            try {
                ...
                driver.get("https://www.binance.com/trade.html?symbol=ETH_USDT");
...
//                    System.out.println(String.format("============================%s", price));
                    LOGGER.info(String.format("ETH RMB price:%s元", priceYuan));
                    EthCommonData data = new EthCommonData();
