...
import com.chengxi.kline.dao.RxpCommonDataMapper;
...
public class BinanceRxpJob {
    ...
    @Autowired
    private RxpCommonDataMapper rxpCommonDataMapper;

    @PostConstruct
    private void doJob(){
        taskExecutor.execute(new RxpSpiderJob(rxpCommonDataMapper));
    }

    static class RxpSpiderJob implements Runnable{

        RxpCommonDataMapper mapper;

        RxpSpiderJob(RxpCommonDataMapper rxpCommonDataMapper){
            mapper = rxpCommonDataMapper;
        }
        ...
        public void run() {
            ...
            try {
                ...
                driver.get("https://www.binance.com/trade.html?symbol=XRP_ETH");
            ...
            while (System.currentTimeMillis() < endTime) {
                ...
                if (StringUtils.hasText(priceFen)) {
                    LOGGER.info(String.format("RXP RMB price:%s元", priceYuan));
                    RxpCommonData data = new RxpCommonData();
